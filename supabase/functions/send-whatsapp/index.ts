/**
 * ============================================================
 * ARCHITECTURE DOCUMENTATION — send-whatsapp
 * ============================================================
 *
 * Request path:
 *   Browser → supabase.functions.invoke("send-whatsapp")
 *          → This Edge Function
 *          → n8n webhook at https://n8n.ixcsoft.com.br/webhook/cotizacion-andinalink
 *
 * Triggered by:
 *   - src/pages/MisCotizaciones.tsx (sendWhatsApp function)
 *   - src/components/QuoteShare.tsx (handleSendWhatsApp function)
 *
 ...
 *   - WAF (Web Application Firewall) rules triggering on payload patterns
...
async function resilientFetch(
  ...
  for (let attempt = 1; attempt <= maxRetries + 1; attempt++) {
    ...
    console.log(`[send-whatsapp] Retry ${attempt}/${maxRetries}, waiting ${Math.round(delay)}ms`);
...
async function logWebhookError(
  ...
  try {
    ...
    await sb.from("webhook_errors").insert({
      ...
      user_agent: "Lovable-EdgeFunction/2.0 send-whatsapp",
    });
  } catch (logErr) {
    console.error("[send-whatsapp] Failed to log webhook error:", logErr);
  }
}
